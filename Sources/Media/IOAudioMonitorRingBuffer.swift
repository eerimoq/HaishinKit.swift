...
class IOAudioMonitorRingBuffer {
    ...
    init?(
        ...
        bufferCounts: UInt32 = IOAudioMonitorRingBuffer.bufferCounts
    ) {
        guard
            inSourceFormat.mFormatID == kAudioFormatLinearPCM,
            let format = AudioCodec.makeAudioFormat(&inSourceFormat),
            let workingBuffer = AVAudioPCMBuffer(pcmFormat: format, frameCapacity: Self.numSamples)
        else {
            return nil
        }
        guard let buffer = AVAudioPCMBuffer(pcmFormat: format, frameCapacity: Self.numSamples * bufferCounts)
        else {
            return nil
        }
        self.format = format
    ...
    func appendSampleBuffer(_ sampleBuffer: CMSampleBuffer, offset: Int = 0) {
        ...
        if format.isInterleaved {
            ...
            case .pcmFormatInt16:
                memcpy(
                    buffer.int16ChannelData?[0].advanced(by: head * channelCount),
                    workingBuffer.int16ChannelData?[0].advanced(by: offset * channelCount),
                    numSamples * channelCount * 2
                )
            case .pcmFormatInt32:
                memcpy(
                    buffer.int32ChannelData?[0].advanced(by: head * channelCount),
                    workingBuffer.int32ChannelData?[0].advanced(by: offset * channelCount),
                    numSamples * channelCount * 4
                )
            case .pcmFormatFloat32:
                memcpy(
                    buffer.floatChannelData?[0].advanced(by: head * channelCount),
            ...
            for i in 0 ..< Int(format.channelCount) {
                ...
                case .pcmFormatInt16:
                    memcpy(
                        buffer.int16ChannelData?[i].advanced(by: head),
                        workingBuffer.int16ChannelData?[i].advanced(by: offset),
                        numSamples * 2
                    )
                case .pcmFormatInt32:
                    memcpy(
                        buffer.int32ChannelData?[i].advanced(by: head),
                        workingBuffer.int32ChannelData?[i].advanced(by: offset),
                        numSamples * 4
                    )
                case .pcmFormatFloat32:
                    memcpy(
                        buffer.floatChannelData?[i].advanced(by: head),
        ...
        if head == buffer.frameLength {
            head = 0
            if sampleBuffer.numSamples - numSamples > 0 {
                ...
                offset: Int = 0) -> OSStatus
    {
        ...
        if format.isInterleaved {
            ...
            case .pcmFormatInt16:
                memcpy(
                    bufferList[0].mData?.advanced(by: offset * channelCount * 2),
                    buffer.int16ChannelData?[0].advanced(by: tail * channelCount),
                    numSamples * channelCount * 2
                )
            case .pcmFormatInt32:
                memcpy(
                    bufferList[0].mData?.advanced(by: offset * channelCount * 4),
                    buffer.int32ChannelData?[0].advanced(by: tail * channelCount),
                    numSamples * channelCount * 4
                )
            case .pcmFormatFloat32:
                memcpy(
                    bufferList[0].mData?.advanced(by: offset * channelCount * 4),
                    buffer.floatChannelData?[0].advanced(by: tail * channelCount),
                    numSamples * channelCount * 4
                )
            default:
                break
            ...
            for i in 0 ..< Int(format.channelCount) {
                ...
                case .pcmFormatInt16:
                    memcpy(
                        bufferList[i].mData?.advanced(by: offset * 2),
                        buffer.int16ChannelData?[i],
                        numSamples * 2
                    )
                case .pcmFormatInt32:
                    memcpy(
                        bufferList[i].mData?.advanced(by: offset * 4),
                        buffer.int32ChannelData?[i],
                        numSamples * 4
                    )
                case .pcmFormatFloat32:
                    memcpy(
